...
*/
    ...
    var config = {
        ...
        // 插入网络视频的校验
        linkVideoCheck: function linkVideoCheck(src) {
            // src 即视频的地址
            return true; // 返回 true 即表示成功
            // return '校验失败'  // 返回字符串即表示失败的提示信息
        },
        ...
        // 插入网络视频的回调
        linkVideoCallback: function linkVideCallback(url) {
...
// 原型
    Video.prototype = {
        ...
        _createPanel: function _createPanel() {
            ...
            if (config.insertWebVideo) {
                // 显示“插入视频”
                tabsConfigResult.push(tabsConfig[0]);
            }
            if (config.uploadLocalVideo || editor.customConfig.uploadLocalVideo) {
                // 显示“上传本地视频”
...
// 原型
    UploadVideo.prototype = {
        ...
        uploadVideo: function uploadVideo(files) {
            ...
            var uploadLocalVideo = config.uploadLocalVideo;

            var maxSize = config.uploadVideoMaxSize;       //100M
            var maxSizeM = maxSize / 1000 / 1000;
            var maxLength = config.uploadVideoMaxLength;
            var uploadFileName = config.uploadVideoFileName;
            var uploadVideoParams = config.uploadVideoParams || {};
            var uploadVideoParamsWithUrl = config.uploadVideoParamsWithUrl;
            var uploadImgHeaders = config.uploadVideoHeaders;
            var hooks = config.uploadVideoHooks || {};
            var timeout = config.uploadVideoTimeout;        //5 min
            ...
            arrForEach(files, function (file) {
                ...
                if (/\.(mp4|ogg|webm)$/i.test(name) === false) {
                    // 后缀名不合法，不是视频
            ...
            if (uploadVideoServer && typeof uploadVideoServer === 'string') {
                // 添加参数
                var uploadVideoServerArr = [];
                uploadVideoServerArr = uploadVideoServer.split('#');
